...
export class AST {
    ...
    toReason(node: Any): string {
        ...
        switch (node.operator) {
            ...
            case "!==":
            case "!=": {
                let reason = field?.name ?? leftString;
...
const conditionalFields = [
    { path: "description", name: "Nội dung yêu cầu" },
    { path: "name", name: "Tên yêu cầu" },
    { path: "status.id", name: "Trạng thái" },
    { path: "requester.id", name: "Người yêu cầu" },
    { path: "type.id", name: "Loại yêu cầu" },
    { path: "group.id", name: "Nhóm hỗ trợ" },
    { path: "technician.id", name: "Cán bộ hỗ trợ" },
    { path: "service.id", name: "Dịch vụ cấp 1" },
    { path: "sub_service.id", name: "Dịch vụ cấp 2" },
    { path: "priority.id", name: "Mức độ ưu tiên" },
    {
        path: "resolution.solution.content",
        name: "Giải pháp cho yêu cầu",
    },
    {
        path: "resolution.cause.content",
        name: "Nguyên nhân của yêu cầu",
    },
    { path: "note", name: "Lý do chuyển trạng thái" },
];
