...
import { GetDepartmentResBody, RoleType } from "../interfaces/response";
...
export async function getPossibleUrgencies(
    tenant?: string
): Promise<{ id: string; name: string }[]> {
    const type = "DEFAULT";
    const match: FilterQuery<IChannel> = {
    ...
    tenant && (match.$or = [{ tenant }, { type }]);
    const channels = await Urgency.aggregate([
        { $match: match },
        {
            $project: {
                _id: 0,
...
export async function getPossibleImpacts(
    tenant?: string
): Promise<{ id: string; name: string }[]> {
    const match: FilterQuery<IChannel> = {
        is_active: true,
    ...
    tenant && (match.$or = [{ tenant }]);
    const priorities = await Impact.aggregate([
        { $match: match },
        {
            $project: {
                _id: 0,
