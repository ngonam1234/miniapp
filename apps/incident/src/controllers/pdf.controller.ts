...
export async function exportSLAPdf(params: {
    ...
    const columns = [
        ...
        { text: "Mức độ ảnh hưởng", style: "column" },
        { text: "Mức độ khẩn cấp", style: "column" },
    ...
    if (params.sla !== undefined) {
        headerContent =
            "BÁO CÁO CAM KẾT CHẤT LƯỢNG DỊCH VỤ (SLA) YÊU CẦU XỬ LÝ SỰ CỐ";
    } else {
        headerContent = "BÁO CÁO TẤT CẢ YÊU CẦU XỬ LÝ SỰ CỐ";
    }
...
export async function exportPdf(params: {
    ...
    const header = [
        { text: "BÁO CÁO TẤT CẢ YÊU CẦU XỬ LÝ SỰ CỐ", style: "header" },
        {
            text: `Kỳ báo cáo: từ ${toFormat(params.start)} đến ${toFormat(
                params.end
            )}`,
...
function getContentHTML(htmlString: string): string {
    ...
    elements.forEach((element) => {
        console.log(
            "🚀 ~ file: pdf.controller.ts:766 ~ elements.forEach ~ element:",
            element
        );
