...
export async function getSubServicesL2(params: { tenant?: string }): Promise<{
    body?: GetSubServiceResBody;
    status: HttpStatus;
    path: string;
}> {
    const url = `${configs.services.service.getUrl()}`;
    try {
        // const param = params.sub_services && `${params.sub_services}/`;
        const res = await axios.get<GetSubServiceResBody>(
            `${url}/sub-services/sub-services-l2?tenant=${params.tenant}`
        );
        return { body: res.data, path: url, status: res.status };
    } catch (e) {
        if (axios.isAxiosError(e) && e.response?.status) {
