...
export async function updateTicketRequest(params: {
    ticketId: string;
...
export async function getTicketRequest(id: string): Promise<{
    body?: unknown;
    status: HttpStatus;
    path: string;
}> {
    const url = `${configs.services.request.getUrl()}/tickets/${id}`;
    try {
