...
export async function updateTicket(
    ...
    if (response.status === HttpStatus.OK) {
        ...
        if (roles && roles.length > 0) {
            isEdit = roles.some((role) => role.request_permission.edit);
            ...
            isEditPriority = roles.some(
                (role) => role.request_permission.edit_priority
            );
            isEditDueDate = roles.some(
                (role) => role.request_permission.edit_due_date
            );
            isEditRequester = roles.some(
                (role) => role.request_permission.edit_requester
            );
            isEditChangeStatusToResolved = roles.some(
                (role) => role.request_permission.change_status_to_resolved
            );
            isEditChangeStatusToClosed = roles.some(
                (role) => role.request_permission.change_status_to_closed
            );
            isEditChangeStatusToCancelled = roles.some(
                (role) => role.request_permission.change_status_to_cancelled
            );
        }
    }
    if (!isEdit) {
    ...
    if (oldTicket?.status?.id !== params?.fields?.status) {
        if (
            (params?.fields?.status ===
                "89e271b0-ad11-11ed-afa1-0242ac120002" &&
                !isEditChangeStatusToResolved) ||
            (params?.fields?.status ===
                "89e27354-ad11-11ed-afa1-0242ac120002" &&
                !isEditChangeStatusToClosed) ||
            (params?.fields?.status ===
                "89e2748a-ad11-11ed-afa1-0242ac120002" &&
