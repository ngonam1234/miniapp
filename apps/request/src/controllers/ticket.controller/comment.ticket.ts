...
export async function commentTicket(
    ...
    const comment = {
        ...
        content: params.content.replace(/ +(?= )/g, "").trim(),
        ...
        activities: [
            {
                ...
                time: new Date(),
                new: params.content.replace(/ +(?= )/g, "").trim(),
            },
        ],
    };
...
export async function editComment(
    ...
    if (oldComment) {
        ...
        const update = {
            $set: {
                "activities.$.comment.content": params.content.replace(/ +(?= )/g, "").trim(),
                "activities.$.comment.attachments": params.attachments,
            },
            $push: {
    ...
    const result = await Ticket.aggregate(pipeline)
        ...
        .then(async (res) => {
            ...
            for (let i = 0; i < info.length; i++) {
                const e = info[i];
                const attachments = await getFileAttachments(e.attachments);
                Object.assign(e, { attachments });
            }
            return {
