...
export async function getSubServicesL2(params: {
    sub_servies: string;
}): Promise<{
    body?: GetSubServicesL2ResBody;
    status?: HttpStatus;
    path: string;
}> {
    const url = `${configs.services.service.getUrl()}`;
    try {
        const res = await axios.get<GetSubServicesL2ResBody>(
            `${url}/sub-services/${params.sub_servies}/sub-services-l2`
        );
        return { body: res.data, path: url };
    } catch (e) {
        if (axios.isAxiosError(e) && e.response?.status) {
